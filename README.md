...
Everything else — company mapping, the team-rename rule, the open-vendor
...
vendors.py            ← open-vendor offset rule (unchanged)
...
- the row's **Vendor is `Concert Seasons` or `Ticketmaster AM`**.
...
Ticketmaster AM check sees the **raw** vendor names — a `Ticketmaster AM` row
gets its order number blanked even though the vendor later resolves to a team
or venue. Other alphanumeric order numbers (e.g. `N4Q-KZGM9MZ`) are kept.
